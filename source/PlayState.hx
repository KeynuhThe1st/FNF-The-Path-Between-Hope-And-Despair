...
class PlayState extends MusicBeatState
{
	...
	override public function update(elapsed:Float)
	{
		...
		if (FlxG.keys.anyJustPressed(debugKeysChart) && !endingSong && !inCutscene && TitleState.konamiUnlocked)
		{
			openChartEditor();
		}
		...
		if (FlxG.keys.anyJustPressed(debugKeysCharacter) && !endingSong && !inCutscene && TitleState.konamiUnlocked) {
	...
	public function triggerEventNote(eventName:String, value1:String, value2:String) {
		switch(eventName) {
			case 'Silhouette':
				var val:Bool = (value1.toLowerCase() == 'on');
				silhouetteBlack.visible = val;

				var chars:Array<Character> = [boyfriend, gf, dad];
				if (val)
				{
					var bgColor:FlxColor = (value2.toLowerCase() == 'white') ? FlxColor.WHITE : FlxColor.BLACK;
					var charColor:FlxColor = (value2.toLowerCase() == 'white') ? FlxColor.BLACK : FlxColor.WHITE;

					silhouetteBlack.color = bgColor;
				...
				else
				{
					for (char in chars)
					{
						if (char != null)
						{
							char.color = FlxColor.WHITE; // Resets to default tint
						}
					}
				}
			case 'Dadbattle Spotlight':
	...
	public function endSong():Void
	{
		...
		if(ret != FunkinLua.Function_Stop && !transitioning) {
			...
			{
				trace('yo whats up guys, its quandale dingle here');
				WeekData.loadTheFirstEnabledMod();
				cancelMusicFadeTween();
				if(FlxTransitionableState.skipNextTransIn) {
					CustomFadeTransition.nextCamera = null;
	...
	override function stepHit()
	{
		...
		if (ClientPrefs.coolCamShake && camZooming) {
			if (curStep % 4 == 0) {
	...
	override function beatHit()
	{
		...
		if(coolcamshakeCheck() && ClientPrefs.coolCamShake && camZooming)
		{
			if (curBeat % 2 == 0) {
				angleshit = anglevar;
			}else {
	...
	function coolcamshakeCheck():Bool
	{
		return ClientPrefs.coolCamShake;
	}
