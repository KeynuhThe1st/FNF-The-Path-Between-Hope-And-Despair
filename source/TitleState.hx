...
class TitleState extends MusicBeatState
{
	...
	public static var konamiUnlocked:Bool = false;
	...
	var konami:Array<FlxKey> = [UP, UP, DOWN, DOWN, LEFT, RIGHT, LEFT, RIGHT, B, A];
	var konamiCounter:Int = 0;
	...
	function startIntro()
	{
		...
		//var gfDance:FlxSprite = new FlxSprite(titleJSON.gfx, titleJSON.gfy);
	...
	override function update(elapsed:Float)
	{
		...
		if (FlxG.keys.firstJustPressed() != FlxKey.NONE && !transitioning && skippedIntro)
		{
			if (FlxG.keys.firstJustPressed() == konami[konamiCounter])
			{
				konamiCounter++;
			}
			else
			...
			{
				TitleState.konamiUnlocked = true;
	...
	{
		// It is generally not a good idea to pause on the title screen.
		// This prevents auto-pause behavior if the setting is enabled.
		if (ClientPrefs.autoPause) return;
		super.onFocusLost();
	}
}
